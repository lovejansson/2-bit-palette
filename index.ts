...
function applyLospecPalette(palette: LospecPalette, colors: PaletteColor[], ctx: CanvasRenderingContext2D, indices: number[]) {
...
async function main() {
    ...
    const inputLospecSearch: SlInput | null = document.getElementById("input-lospec-search") as SlInput | null;
    const btnLospecSearch: SlButton | null = document.getElementById("btn-lospec-search") as SlButton | null;
    const btnApplyLospecPalette: SlButton | null = document.getElementById("btn-apply-lospec-palette") as SlButton | null;
    const divLospecPalette: HTMLDivElement = document.getElementById("lospec-palette")! as HTMLDivElement;
    const pLospecPalette: HTMLParagraphElement = document.getElementById("lospec-palette-name")! as HTMLParagraphElement;
    const pLospecPaletteAuthor: HTMLParagraphElement = document.getElementById("lospec-palette-author")! as HTMLParagraphElement;
    const divLospecError: HTMLDivElement = document.getElementById("lospec-search-error")! as HTMLDivElement;
    const pLospecError: HTMLDivElement = document.getElementById("lospec-error")! as HTMLParagraphElement;
    ...
    if (
        ...
        dialogTag === null ||
        btnMoon === null ||
        btnSun === null || btnLospecSearch === null || inputLospecSearch === null || btnApplyLospecPalette === null
    )
    ...
    btnApplyLospecPalette.addEventListener("click", () => {
        divLospecPalette.parentElement?.classList.add("display-none");
        if (lospecPalette !== null) { applyLospecPalette(lospecPalette, colors, ctx, indices); }

    });

    inputLospecSearch.addEventListener("input", () => {
        if (!divLospecError.classList.contains("display-none")) {
            divLospecError.classList.add("display-none");
        }

        if (!divLospecPalette.parentElement?.classList.contains("display-none")) {
            divLospecPalette.parentElement?.classList.add("display-none");
        }
    });
        ...
        if (inputLospecSearch.value) {
            ...
            if (palette) {
                if (palette.colors.length !== 4) {
                    divLospecError.classList.remove("display-none");
                    pLospecError.textContent = "Palette must have exactly 4 colors."
                } else {
                    ...
                    divLospecPalette.parentElement?.classList.remove("display-none");
                }


            } else {
                divLospecError.classList.remove("display-none");
                pLospecError.textContent = "Palette not found."
            }
